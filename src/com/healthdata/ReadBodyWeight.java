...
// Main class to read and process Body Weight data
public class ReadBodyWeight {

    // Method to process files and perform the RML mapping for each
    public void processFiles() throws IOException {
        String dataFolder = "schema\\Body Weight\\"; // Path to the JSON scemas and tamplate rml file
        String templateRmlFileName = dataFolder + "body_weight_temp.ttl"; // Name of the RML template file
        ...
        Files.list(Paths.get(dataFolder))
                ...
                .forEach(path -> {
                    try {
                        ...
                        String NewRmlFileName = dataFolder + "body_weight_" + number + "_" + username + ".ttl"; // The new .ttl file with RML rules for each json file.

                        // Parse the JSON file
                        JSONObject json = new JSONObject(new JSONTokener(new FileReader(path.toString())));
                        String weightUnit = json.getJSONObject("body").getJSONObject("body_weight").getString("unit"); // Get the unit of weight from JSON file
                        String unitForBodyWeight;

                        // Map units to specific QUDT ontology unit names
                        switch(weightUnit) {
                            case "kg":
                                unitForBodyWeight = "KiloGM";
                                break;
                            case "lbm":
                            case "lb":
                                unitForBodyWeight = "LB";
                                break;
                            default:
                                unitForBodyWeight = "Unknown";
                        }

                        // Replace input.json, User, and UnitForBodyWeight that are used on template RML file with actual values for the new RML files
                        String newContent = templateContent.replace("input.json", jsonFileName)
                                .replace("User", username)
                                .replace("UnitForBodyWeight", unitForBodyWeight);
    ...
    void runRMLMapper(String path, String username, String number) throws IOException, InterruptedException {
        String inputFileName = "body_weight_" + number + "_" + username + ".ttl";
        String outputFileName = "body_weight_" + number + "_" + username + "_output.ttl";
