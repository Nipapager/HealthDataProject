...
// Main class to read and process Calories Burned data
public class ReadCaloriesBurned {

    // Method to process files and perform the RML mapping for each
    public void processFiles() throws IOException {
        String dataFolder = "schema\\Calories Burned\\"; // Path to the JSON scemas and tamplate rml file
        String templateRmlFileName = dataFolder + "calories_burned_temp.ttl"; // Name of the RML template file
        ...
        Files.list(Paths.get(dataFolder))
                ...
                .forEach(path -> {
                    try {
                        ...
                        String NewRmlFileName = dataFolder + "calories_burned_" + number + "_" + username + ".ttl"; // The new .ttl file with RML rules for each json file.

                        // Parse the JSON file
                        JSONObject json = new JSONObject(new JSONTokener(new FileReader(path.toString())));
                        String kcalBurnedUnit = json.getJSONObject("body").getJSONObject("kcal_burned").getString("unit"); // Get the unit of Calories Burned from JSON file

                        // Map units to specific QUDT ontology unit names
                        String unitForCalBurn = "kcal".equals(kcalBurnedUnit) ? "KiloCAL" : "CAL_MEAN";

                        // Replace input.json, User, and UnitForCalBurn that are used on template RML file with actual values for the new RML files
                        String newContent = templateContent.replace("input.json", jsonFileName)
                                .replace("User", username)
                                .replace("UnitForCalBurn", unitForCalBurn);
    ...
    void runRMLMapper(String path, String username, String number) throws IOException, InterruptedException {
        String inputFileName = "calories_burned_" + number + "_" + username + ".ttl";
        String outputFileName = "calories_burned_" + number + "_" + username + "_output.ttl";
