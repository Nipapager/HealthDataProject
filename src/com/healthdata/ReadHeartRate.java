...
// Main class to read and process Heart Rate data
public class ReadHeartRate {

    // Method to process files and perform the RML mapping for each
    public void processFiles() throws IOException {
        String dataFolder = "schema\\Heart Rate\\"; // Path to the JSON scemas and tamplate rml file
        String templateRmlFileName = dataFolder + "heart_rate_temp.ttl"; // Name of the RML template file
        ...
        Files.list(Paths.get(dataFolder))
                ...
                .forEach(path -> {
                    try {
                        ...
                        String NewRmlFileName = dataFolder + "heart_rate_" + number + "_" + username + ".ttl"; // The new .ttl file with RML rules for each json file.

                        // Parse the JSON file
                        JSONObject json = new JSONObject(new JSONTokener(new FileReader(path.toString())));
                        String heartRateUnit = json.getJSONObject("body").getJSONObject("heart_rate").getString("unit");

                        // Map units to specific QUDT ontology unit names
                        String unitForHeartRate = "beats/min".equals(heartRateUnit) ? "BEAT-PER-MIN" : "HZ";

                        // Replace input.json, User, and UnitForHeartRate that are used on template RML file with actual values for the new RML files
                        String newContent = templateContent.replace("input.json", jsonFileName)
                                .replace("User", username)
                                .replace("UnitForHeartRate", unitForHeartRate);
    ...
    void runRMLMapper(String path, String username, String number) throws IOException, InterruptedException {
        String inputFileName = "heart_rate_" + number + "_" + username + ".ttl";
        String outputFileName = "heart_rate_" + number + "_" + username + "_output.ttl";
